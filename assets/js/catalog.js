      ...
      <div>
        <div class="card p-4 text-center rounded-0">
          ...
          <div class="card-body">
            ...
            <button class="btn btn-success add-to-cart-btn" data-id="${bookID}">Add Cart</button>
          </div>
        </div>
      </div>
      `;
...
$(document).ready(function () {
  ...
  function updateCartSummary() {
    ...
    const totalPrice = totalItems * 50;
  ...
  $(".add-to-cart-btn").on("click", async function (e) {
    ...
    const selectedBookJson = await fetchBookDetails(bookId);
    addToCart(selectedBookJson);
  });
  ...
  function addToCart(selectedBookJson) {
    let bookId = selectedBookJson.id;

    // Get or initialize cart data from local storage
    const cart = JSON.parse(localStorage.getItem("cart")) || [];
